...
if (isset($uid)) {
    ...
    $sql = "SELECT cities.*, c_usr.name as cname, c_usr.sname as csname, 
        u_usr.name as uname, u_usr.sname as usname  
        FROM cities as cities
        LEFT JOIN users as c_usr ON c_usr.id = cities.created_by 
        LEFT JOIN users as u_usr ON u_usr.id = cities.updated_by
        ORDER BY cities.id DESC";
    $result = mysqli_query($conn, $sql);

    $provinceSql = "SELECT * FROM provinces ORDER BY english ASC";
    $provinceData = mysqli_query($conn, $provinceSql);

    $settings_menu_active = "active";
    $city_menu_active = "active";
    $pageTitle = "City List";
?>
        ...
        <div class="container-fluid-lg">
            ...
            <div class="dt-list-item-block">
                <div class="" style="text-align: right; margin-bottom:10px;">
                    <a href="javascript:void(0);" class="btn btn-success open-city-btn"><i class="fa fa-plus" aria-hidden="true"></i> Add</a>
                </div>
                <table class="table table-hover item-list-dataTable">
                    <thead>
                        <tr>
                            ...
                            <th onclick="sortTable(2)">Province Name</th>
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <a title="<?php echo $row["city_name"]; ?>" class="btn-edit-sale" href="city/<?php echo $row["id"]; ?>">
                                ...
                                <td class="text-nowrap table-action-col">
                                    <div class="btn-group table-btn-group order-table-action-container">
                                        <a title="Edit" class="btn-edit-sale" href="city/<?php echo $row["id"]; ?>">
                                            <i class="fa-sharp fa-solid fa-pencil"></i>
                                        </a>
                                        <a title="Delete" class="btn-delete-sale delete" href="city/delete/<?php echo $row["id"]; ?>">
            ...
            <div class="slide-order-form-block">
                <?php require_once(VIEW_PATH . 'settings/city/city-form.php');
                ?>
            </div>
    ...
    <script src="<?php echo Helper::fullbaseUrl(); ?>assets/js/city.js"></script>
