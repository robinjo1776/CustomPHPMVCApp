$(document).ready(function () {
  ...
  $(".item-list-dataTable").on("click", ".btn-edit-sale", function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#status_unit").val(response.data.status_unit);
        $("#unit_id").val(response.data.id);

        $("#savefrmBtn").text("Update");
        $(".slider-form-header").text("Update Unit");
      },
  ...
  $(".slide-form-submit").on("submit", function (e) {
    ...
    if ($("#slide_new_form").valid()) {
      ...
      url = API_BASE_URL + "unit/create";
      let unit_id = $("#unit_id").val();
      if (unit_id) {
        url = API_BASE_URL + "unit/update";
      }
